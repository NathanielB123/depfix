...
TmPreD : ∀ {CtxR TyR TyPreR ,×TmUR} 
           (TmPreR : ∀ Γ → TyPreD TyPreR ,×TmUR Γ → Set)
           (Π×wkR : ∀ Γ (A : TyPreR Γ) 
                  → (TyPreD TyPreR ,×TmUR (lower (proj₁ (,×TmUR Γ A)))
                  → TyPreD TyPreR ,×TmUR Γ) 
                  × (TyPreR Γ
                  → TyPreD TyPreR ,×TmUR (lower (proj₁ (,×TmUR Γ A)))))
       → ∀ (Γ : CtxD CtxR TyR) → TyPreD TyPreR ,×TmUR Γ → Set 
TmPreD {,×TmUR = ,×TmUR} TmPreR Π×wkR Γ A
  = (∃ λ A′ → ∃ λ B′ → TmPreR _ B′ × A ≡ ΠR Γ A′ B′)
  + (∃ λ Γ′ → ∃ λ A′ → ∃ λ B′ → TmPreR _ (ΠR Γ′ A′ B′) 
     × Σ (Γ ≡ ,R Γ′ A′) λ Γ≡ → A ≡[ cong (TyPreD _ _) Γ≡ ]≡ B′)
  + (∃ λ Γ′ → ∃ λ A′ → Σ (Γ ≡ ,R Γ′ A′) λ Γ≡ 
     → A ≡[ cong (TyPreD _ _) Γ≡ ]≡ wkR Γ′ A′ A′)  
  where ,R = λ Γ A → lower (proj₁ (,×TmUR Γ A))
        ΠR = λ Γ A → proj₁ (Π×wkR Γ A)
        wkR = λ Γ A → proj₂ (Π×wkR Γ A)

pattern lam M = inl (_ Σ, _ Σ, M Σ, refl)
pattern app M = inm (_ Σ, _ Σ, _ Σ, M Σ, refl Σ, refl)
pattern vz = inr (_ Σ, _ Σ, refl Σ, refl)

TmPre : ∀ {CtxR TyR TyPreR ,×TmUR}  (Γ : CtxD CtxR TyR) → TyPreD TyPreR ,×TmUR Γ 
      → Set
...
instance
  TyPreD-RecFunctor : RecFunctor (CtxD Ctx (fixInterpret tt)) Set _ TyPreD
...
Tm Γ A = TmPre {,×TmUR = fixInterpret} (unfix Γ) (unfix A)
